...
.controller('BookCtrl', ['$scope', '$state', 'LSFactory', 'AuthFactory', '$rootScope', 'UserFactory', 'Loader',
    function($scope, $state, LSFactory, AuthFactory, $rootScope, UserFactory, Loader) {
      ...
      // $scope.book_list = [];
      //  var book_list = UserFactory.getCartItems();
//end of fetch cart length
...
//login to get cart total price
                ...
                $scope.cart = [];
                   ...
                   for (var i = 0; i < _cart.length; i++) {
                        ...
                        cart_sum += (parseInt(_cart[i].price) * parseInt(_cart[i].qty));
                    };
                    $scope.cart.total = cart_sum;
        ...
        $scope.checkout = function() {
           // we need to send only the id and qty
            ...
            for (var i = 0; i < _cart.length; i++) {
                cart.push({
                    id: _cart[i]._id,
                    qty: parseInt(_cart[i].qty)// hardcoded to 1
            ...
            UserFactory.getPurchases().success(function(data) {
                ...
                for (var i = 0; i < purchases.length; i++) {
                    ...
                    for (var j = 0; j < purchases[i][key].length; j++) {
                        sum += (parseInt(purchases[i][key][j].price) * parseInt(purchases[i][key][j].qty));
                    };
